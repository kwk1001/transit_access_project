...
peek_java_config <- function(config_path, default_memory = "12G", default_active_processors = 2L) {
  ...
  `%notnull%` <- function(a, b) if (!is.null(a)) a else b

  update_from_cfg <- function(cfg_obj) {
    if (!is.list(cfg_obj)) {
      return(invisible(NULL))
    }

    java_cfg <- cfg_obj$r5r_network %notnull% cfg_obj$routing %notnull% list()
    mem <- java_cfg$java_memory %notnull% cfg_obj$java_memory
    ap <- java_cfg$java_active_processors %notnull% cfg_obj$java_active_processors

    if (!is.null(mem) && nzchar(as.character(mem))) {
      out$java_memory <<- as.character(mem)
    }
    if (!is.null(ap) && is.finite(suppressWarnings(as.numeric(ap)))) {
      out$java_active_processors <<- as.integer(ap)
    }
  ...
  if (ext %in% c("yml", "yaml")) {
    if (requireNamespace("yaml", quietly = TRUE)) {
      ...
      update_from_cfg(cfg_yaml)
    }

    if (identical(out$java_memory, default_memory) && identical(out$java_active_processors, default_active_processors)) {
