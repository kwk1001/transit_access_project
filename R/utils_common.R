...
standardize_zone_id <- function(x, unit = "tract") {
  ...
  if (unit_use %in% c("zip", "zipcode", "zcta")) {
    ...
    out <- dplyr::case_when(
      ...
      n_digits <= 5 ~ stringr::str_pad(digits, width = 5, side = "left", pad = "0"),
      n_digits == 9 ~ stringr::str_sub(digits, 1, 5),
      TRUE ~ NA_character_
    )
    return(out)
  }
...
csv_gz_has_data_rows <- function(path) {
...
all_csv_gz_have_data_rows <- function(paths) {
  if (length(paths) == 0) {
    return(FALSE)
  }
  all(vapply(paths, csv_gz_has_data_rows, logical(1)))
}
