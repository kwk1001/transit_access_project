...
### Q5：ZIP 模式 Boston 一直有少数 zone 报 routing Java 错误怎么办

A：新版本会在运行日志目录输出 `zone_point_qc.csv`，其中包含每个 zone 的路由代表点来源（如 `nearest_tract_centroid_to_zone_mean` 或 fallback）。  
如果某些 zone 反复失败，请先检查该文件，再查看 `routing_origin_fallback_*.json` 中的失败 origin 列表。

### Q6：怎么系统定位“到底是点位问题还是网络/数据问题”

A：使用诊断脚本：  
`Rscript scripts/08_diagnose_routing_failures.R <config_path> [source_id] [analysis_unit]`  
脚本会读取 fallback 日志中的失败 origin，自动执行单 origin 的 1x1 / 1x10 路由探针，并输出 `routing_origin_probe_results.csv` 到 `logs_dir`。
