...
message("Run ID: ", cfg$run$run_id)
message("Analysis signature: ", cfg$run$analysis_signature_short)
message("Run outputs root: ", cfg$paths$run_root)
message("Maps output root: ", cfg$paths$maps_dir)

run_download_geography(cfg)
run_download_optional_covariates(cfg)
run_standardize_surveys(cfg, source_ids = args$source_id)
run_build_od_weights(cfg, source_id = args$source_id)
